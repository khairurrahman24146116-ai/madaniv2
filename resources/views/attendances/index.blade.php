...
{{-- Filters --}}
<form action="{{ route('attendances.index') }}" method="GET" class="flex flex-wrap gap-md mb-lg p-md bg-surface-container-low rounded-xl border border-outline-variant">
    <div>
    ...
    <div class="self-end flex gap-sm">
        ...
        <a href="{{ route('attendances.index') }}" class="px-lg py-2 border border-outline-variant rounded-lg text-label-md text-on-surface-variant hover:bg-surface-container transition-colors inline-flex items-center">
            Reset
        </a>
    </div>
</form>
...
<x-card variant="default">
    <div class="p-lg border-b border-outline-variant flex justify-between items-center bg-surface-container-low">
        ...
        <a href="{{ route('attendances.export-csv') }}?{{ http_build_query(request()->query()) }}" class="flex items-center gap-sm px-md py-2 border border-outline-variant rounded-lg text-label-md text-on-surface-variant hover:bg-surface-container transition-colors">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-outline-variant">
                ...
                <tr class="hover:bg-surface-container-low transition-colors">
                    ...
                    <td class="px-lg py-md text-body-md text-on-surface-variant">{{ $a->student->nis }}</td>
                    <td class="px-lg py-md text-body-md text-on-surface font-semibold">{{ $a->student->name }}</td>
                    <td class="px-lg py-md text-body-md text-on-surface-variant">{{ $a->schedule->teacherSubject->classroom->name ?? '-' }}</td>
