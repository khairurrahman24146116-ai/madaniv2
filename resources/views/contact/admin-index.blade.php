        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-surface-container-high border-b border-outline-variant">
                    <th class="px-lg py-md text-label-md text-on-surface-variant uppercase">Pengirim</th>
            ...
            <tbody class="divide-y divide-outline-variant">
                ...
                <tr class="hover:bg-surface-container-low transition-colors {{ !$msg->is_read ? 'font-semibold' : '' }}">
                    <td class="px-lg py-md text-body-md text-on-surface">{{ $msg->user->name }}</td>
                    <td class="px-lg py-md text-body-md text-on-surface">{{ $msg->subject }}</td>
                    <td class="px-lg py-md">
                        @if($msg->admin_reply)
                            <span class="text-green-700 text-label-md">Dibalas</span>
                        @else
                            <span class="text-amber-700 text-label-md">Baru</span>
                        @endif
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="5" class="text-center py-xl text-on-surface-variant">Belum ada pesan masuk.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
