...
<form action="{{ route('wali.contact.store') }}" method="POST" class="max-w-2xl space-y-md">
    @csrf
    <x-card variant="default" padding="lg">
        <div class="space-y-md">
            <div>
                <label class="text-label-md text-on-surface-variant block mb-xs">SUBJEK</label>
            ...
            <div>
                <label class="text-label-md text-on-surface-variant block mb-xs">PESAN</label>
    ...
    <div class="flex gap-sm">
        ...
        <a href="{{ route('wali.contact.index') }}" class="px-lg py-2 border border-outline-variant rounded-lg text-label-md text-on-surface-variant hover:bg-surface-container transition-colors inline-flex items-center">Batal</a>
    </div>
</form>
@endsection
