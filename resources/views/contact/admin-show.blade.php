...
<x-card variant="default" padding="lg" class="mb-lg">
    <p class="text-label-md text-green-700 mb-sm">BALASAN ANDA</p>
    <p class="text-body-lg text-on-surface whitespace-pre-line">{{ $contactMessage->admin_reply }}</p>
    <p class="text-label-md text-on-surface-variant mt-sm">{{ $contactMessage->replied_at ? $contactMessage->replied_at->format('d/m/Y H:i') : '' }}</p>
</x-card>
@endif

@if(!$contactMessage->admin_reply)
<form action="{{ route('admin.contact.reply', $contactMessage) }}" method="POST" class="max-w-2xl space-y-md">
    @csrf
    <x-card variant="default" padding="lg">
        <div>
            <label class="text-label-md text-on-surface-variant block mb-xs">BALASAN</label>
