...
@section('content')
<x-page-header title="Edit Mapping" subtitle="{{ $teacherSubject->user->name }} - {{ $teacherSubject->subject->name }} ({{ $teacherSubject->classroom->name }})" icon="assignment_ind"
...
<x-card variant="default" padding="lg" class="max-w-2xl">
    <form method="POST" action="{{ route('admin.teacher-subjects.update', $teacherSubject) }}">
        @csrf @method('PUT')
        <div class="space-y-5">
            <div>
                ...
                <select name="user_id" required
                    ...
                    <option value="{{ $t->id }}" @selected(old('user_id', $teacherSubject->user_id)==$t->id)>{{ $t->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="subject_id" required
                    ...
                    <option value="{{ $s->id }}" @selected(old('subject_id', $teacherSubject->subject_id)==$s->id)>{{ $s->name }} ({{ $s->code }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="classroom_id" required
                    ...
                    <option value="{{ $c->id }}" @selected(old('classroom_id', $teacherSubject->classroom_id)==$c->id)>{{ $c->name }}</option>
                    @endforeach
                </select>
