...
<x-card variant="default">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-surface-container-high border-b border-outline-variant">
                    ...
                    <th class="px-6 py-4 text-label-md text-on-surface-variant uppercase">Tingkat</th>
                    <th class="px-6 py-4 text-label-md text-on-surface-variant uppercase">Wali Kelas</th>
                    <th class="px-6 py-4 text-label-md text-on-surface-variant uppercase">Tahun Ajaran</th>
                    <th class="px-6 py-4 text-label-md text-on-surface-variant uppercase text-center">Siswa</th>
            ...
            <tbody class="divide-y divide-outline-variant">
                ...
                <tr class="hover:bg-surface-container-low transition-colors">
                    <td class="px-6 py-4 text-body-md text-on-surface font-semibold">{{ $c->name }}</td>
                    <td class="px-6 py-4 text-body-md text-on-surface-variant">{{ $c->grade }}</td>
                    <td class="px-6 py-4 text-body-md text-on-surface">{{ $c->waliKelas?->name ?? '-' }}</td>
                    <td class="px-6 py-4 text-body-md text-on-surface-variant">{{ $c->academic_year }}</td>
                    <td class="px-6 py-4 text-center text-body-md text-on-surface-variant">{{ $c->students_count }}</td>
                    <td class="px-6 py-4 text-right">
                        ...
                        <form action="{{ route('admin.classrooms.destroy', $c) }}" method="POST" class="inline"
                            data-confirm="Hapus kelas {{ $c->name }}?"
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center py-8 text-on-surface-variant">Belum ada kelas.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="p-6 border-t border-outline-variant">{{ $classrooms->links() }}</div>
    @endif
</x-card>
@endsection
