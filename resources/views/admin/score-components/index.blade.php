        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-surface-container-high border-b border-outline-variant">
                    ...
                    <th class="px-lg py-md text-label-md text-on-surface-variant uppercase">Komponen</th>
                    <th class="px-lg py-md text-label-md text-on-surface-variant uppercase">Bobot</th>
                    <th class="px-lg py-md text-label-md text-on-surface-variant uppercase">Semester</th>
            ...
            <tbody class="divide-y divide-outline-variant">
                ...
                <tr class="hover:bg-surface-container-low transition-colors">
                    <td class="px-lg py-md text-body-md text-on-surface font-semibold">{{ $c->subject->name }}</td>
                    <td class="px-lg py-md text-body-md text-on-surface-variant">{{ $c->name }} ({{ $c->code }})</td>
                    <td class="px-lg py-md text-body-md text-on-surface-variant">{{ $c->weight }}%</td>
                    <td class="px-lg py-md text-body-md text-on-surface-variant capitalize">{{ $c->semester }}</td>
                    ...
                    <td class="px-lg py-md text-right">
                        ...
                        <form action="{{ route('admin.score-components.destroy', $c) }}" method="POST" class="inline" onsubmit="return confirm('Hapus bobot {{ $c->name }}?')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="6" class="text-center py-xl text-on-surface-variant">Belum ada konfigurasi bobot nilai.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
