...
@section('content')
<x-page-header title="Tambah Bobot Nilai" subtitle="Atur persentase bobot komponen penilaian" icon="tune"
...
<x-card variant="default" padding="lg" class="max-w-2xl">
    <form method="POST" action="{{ route('admin.score-components.store') }}">
        @csrf
        <div class="space-y-5">
            <div>
            ...
            <div>
                ...
                <select name="code" required
                    ...
                    <option value="tugas" @selected(old('code')=='tugas')>Tugas</option>
                    <option value="ph" @selected(old('code')=='ph')>Penilaian Harian (PH)</option>
                    <option value="uts" @selected(old('code')=='uts')>UTS</option>
                    <option value="uas" @selected(old('code')=='uas')>UAS</option>
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="weight" value="{{ old('weight') }}" required step="0.01" min="0" max="100"
                        class="w-full rounded-lg border border-outline-variant bg-surface-bright text-on-surface px-4 py-3 text-body-md outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors"
                        placeholder="25">
