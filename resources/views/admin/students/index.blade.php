...
<form method="GET" action="{{ route('admin.students.index') }}" class="mb-lg">
    <div class="flex flex-col sm:flex-row gap-md">
        ...
        <button type="submit" class="px-lg py-2.5 rounded-lg bg-primary text-on-primary text-label-md font-semibold hover:bg-primary/90 transition-colors">Cari</button>
        @if(request('search') || request('status') !== null)
            <a href="{{ route('admin.students.index') }}" class="px-lg py-2.5 rounded-lg border border-outline-variant text-label-md font-medium text-on-surface hover:bg-surface-container-high transition-colors text-center">Reset</a>
        @endif
    </div>
</form>
            ...
            <tbody class="divide-y divide-outline-variant">
                ...
                <tr class="hover:bg-surface-container-low transition-colors {{ !$s->is_active ? 'opacity-60' : '' }}">
                    ...
                    <td class="px-lg py-md text-center">
                        @if($s->is_active)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-50 text-green-700 text-label-md font-semibold border border-green-200">
                                <span class="material-symbols-outlined text-[14px]">check_circle</span>
                                Aktif
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-50 text-red-700 text-label-md font-semibold border border-red-200">
                    ...
                    <td class="px-lg py-md text-right whitespace-nowrap">
                        ...
                        <a href="{{ route('admin.students.move-form', $s) }}" class="inline-flex items-center gap-1 text-label-md text-secondary hover:text-secondary/80 ml-md">
                ...
                @empty
                <tr><td colspan="6" class="text-center py-xl text-on-surface-variant">
                    @if(request('search') || request('status') !== null)
