...
@section('content')
<x-page-header title="Edit Jadwal" subtitle="{{ ucfirst($schedule->day) }} {{ $schedule->start_time }}-{{ $schedule->end_time }}" icon="calendar_month"
...
<x-card variant="default" padding="lg" class="max-w-2xl">
    <form method="POST" action="{{ route('admin.schedules.update', $schedule) }}">
        ...
        <div class="space-y-5">
            <div>
                <label class="block text-label-md text-on-surface-variant mb-xs">MAPPING GURU-MAPEL-KELAS</label>
                <select name="teacher_subject_id" required
                    ...
                    <option value="{{ $m->id }}" @selected(old('teacher_subject_id', $schedule->teacher_subject_id)==$m->id)>
            ...
            <div>
                <label class="block text-label-md text-on-surface-variant mb-xs">HARI</label>
                <select name="day" required
                    class="w-full rounded-lg border border-outline-variant bg-surface-bright text-on-surface px-4 py-3 text-body-md outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors">
                    <option value="">Pilih hari</option>
                    @foreach(['senin','selasa','rabu','kamis','jumat','sabtu'] as $day)
                    <option value="{{ $day }}" @selected(old('day', $schedule->day)==$day)>{{ ucfirst($day) }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-label-md text-on-surface-variant mb-xs">JAM MULAI</label>
                    <input type="time" name="start_time" value="{{ old('start_time', $schedule->start_time) }}" min="14:00" max="16:00" required
                        class="w-full rounded-lg border border-outline-variant bg-surface-bright text-on-surface px-4 py-3 text-body-md outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors">
                    @error('start_time')<p class="text-error text-caption mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-label-md text-on-surface-variant mb-xs">JAM SELESAI</label>
                    <input type="time" name="end_time" value="{{ old('end_time', $schedule->end_time) }}" min="14:00" max="16:00" required
                        class="w-full rounded-lg border border-outline-variant bg-surface-bright text-on-surface px-4 py-3 text-body-md outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors">
                    @error('end_time')<p class="text-error text-caption mt-1">{{ $message }}</p>@enderror
                </div>
            </div>
            <div>
                <label class="block text-label-md text-on-surface-variant mb-xs">JAM KE-</label>
                <select name="hour_order" required
                    class="w-full rounded-lg border border-outline-variant bg-surface-bright text-on-surface px-4 py-3 text-body-md outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors">
                    <option value="">Pilih urutan jam</option>
                    @for($i = 1; $i <= 4; $i++)
                    <option value="{{ $i }}" @selected(old('hour_order', $schedule->hour_order)==$i)>{{ $i }}</option>
                    @endfor
                </select>
