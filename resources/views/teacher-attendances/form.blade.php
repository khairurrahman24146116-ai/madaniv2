...
{{-- Status Card --}}
<x-card variant="elevated" padding="lg" class="mb-6 text-center">
    @if($todayAttendance && $todayAttendance->check_in)
        <div class="w-16 h-16 bg-tertiary-fixed/40 text-tertiary-container rounded-full flex items-center justify-center mx-auto mb-4">
            <span class="material-symbols-outlined text-4xl">check_circle</span>
        </div>
        <h2 class="text-headline-xl text-on-surface mb-1">Anda Sudah Check-in</h2>
    ...
    @else
        <div class="w-16 h-16 bg-surface-container text-on-surface-variant rounded-full flex items-center justify-center mx-auto mb-4">
            <span class="material-symbols-outlined text-4xl">person</span>
        </div>
        <h2 class="text-headline-xl text-on-surface mb-1">Belum Check-in</h2>
        <p class="text-body-lg text-on-surface-variant mb-6">
            {{ \Carbon\Carbon::parse(now()->format('Y-m-d'))->locale('id')->isoFormat('dddd, D MMMM YYYY') }}
        </p>
        <form action="{{ route('teacher.attendances.checkin') }}" method="POST" class="flex flex-col items-center gap-4">
            @csrf
            <input type="hidden" name="date" value="{{ now()->format('Y-m-d') }}">
            <div>
                <label class="text-label-md text-on-surface-variant block mb-1">JADWAL (opsional)</label>
...
<x-card variant="default" padding="lg">
    <h3 class="text-headline-md text-on-surface mb-4">Form Absensi Manual</h3>
    <form action="{{ route('teacher.attendances.store') }}" method="POST" class="space-y-4 max-w-xl">
        @csrf
        <input type="hidden" name="date" value="{{ now()->format('Y-m-d') }}">
        <div>
            <label class="text-label-md text-on-surface-variant block mb-1">JADWAL (opsional)</label>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-2 sm:flex sm:gap-2">
                ...
                <label class="flex-1 py-3 px-2 sm:px-3 rounded-lg text-label-md text-center border border-outline-variant cursor-pointer hover:bg-surface-container has-[:checked]:bg-primary has-[:checked]:text-white has-[:checked]:border-primary transition-colors">
        ...
        <div>
            <label class="text-label-md text-on-surface-variant block mb-1">KETERANGAN</label>
